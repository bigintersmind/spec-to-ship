...
If a parent ticket was provided in step 2, after publishing all children, post a single comment on the parent listing the child references. Use this format verbatim — the disclaimer prefix, the `## Children` heading, and a bullet list of child references, in that order:
...
Downstream tooling (e.g. `ralph/review.sh`) parses this comment to walk parent→children, so the heading and bullet-list shape are mandatory.

If you are re-running `/issues` against a parent that already has one or more `## Children` comments from a prior run, post a fresh comment in the format above anyway. Do not detect, overwrite, edit, or remove existing `## Children` comments. Stale comments from prior runs are intentional — downstream tooling is responsible for taking the most recent one. Do not "fix" this by adding deduplication logic.

Do NOT close or modify the parent's body.
