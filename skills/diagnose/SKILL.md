...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), surface that recommendation explicitly — flag it for the maintainer or open a follow-up ticket for an architecture pass. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
