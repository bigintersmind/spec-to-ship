...
An agent brief is a structured comment posted on a ticket when it moves to `ready-for-agent`. It is the authoritative specification that an AFK agent will work from. The original ticket body and discussion are context — the agent brief is the contract.
...
- **Good:** "Running the triage skill with no arguments returns issues that have been through initial classification"
