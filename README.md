...
- **`ralph/review.sh <prd-ref>`** — post-loop review companion. Works for PRDs completed via `afk.sh`, `once.sh`, or hand work — the script auto-detects which checkout has the implementation. The main checkout wins when it has commits ahead of base; otherwise it falls back to the `${REPO_ROOT}-afk` worktree. When HEAD is on the base branch itself (e.g. `once.sh` work committed to `main` before pushing), the main-checkout range switches to `origin/<base>..HEAD` so unpushed commits stay reviewable. Either way, it runs a single non-interactive `claude --verbose --print --output-format stream-json` iteration (output streams live, matching `afk.sh`) that diffs against the base, fetches the PRD body and each child's agent brief, and posts a structured five-question review comment on the PRD ticket (PRD coverage, acceptance criteria status, brief-vs-PRD fidelity, out-of-scope drift, follow-ups). Idempotent — re-runs append a fresh comment with a timestamp + commit-range header so the PRD's history is a chronological log of what each pass found. Existing AFK-installed repos pick up `review.sh` by re-running `/setup-skills`, which will offer an incremental install when `ralph/` is missing only the new files.
