...
- `ralph/review.sh <prd-ref>` — post-loop review companion paired with `ralph/review-prompt.md`. Auto-detects which checkout has the implementation (main when ahead of base; AFK worktree otherwise; `origin/<base>..HEAD` when on the base branch itself), runs one `claude --print` pass that streams output live via the same `jq` filter as `afk.sh`, and posts a structured comment on the PRD ticket. Idempotent — re-runs append. `setup-skills`'s incremental install path installs it into existing AFK consumer repos. See README.md for the full user-facing behavior.
...
## This repo's dogfood configuration

This repo runs the workflow against itself. The stable verbs from the contract section above resolve to:

- **Issue tracker:** GitHub issues via `gh`. See `docs/agents/issue-tracker.md`.
- **Triage labels:** canonical role names used verbatim as label strings. See `docs/agents/triage-labels.md`.
- **Domain docs:** single-context. See `docs/agents/domain.md`.
- **AFK loop:** installed at `ralph/`. `./ralph/once.sh` for a single HITL iteration, `./ralph/afk.sh <N>` for unattended loops on `ready-for-agent` tickets, `./ralph/review.sh <prd-ref>` for post-loop PRD review.

Editing the dogfood config means editing `docs/agents/*.md` here. Editing the seed templates that future consumers get means editing files under `plugin/skills/setup-skills/`.
